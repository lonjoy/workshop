...
$packageHashs = $_POST;//资源包与hash、sum关联数组
$g_res_obj = array();//资源对象

if($g_get_type == WP_GET_LIST){//如果是请求list
    foreach($packageHashs as $package => $value){
        $g_res_obj[] = Bd_Webapp_Reader::getList($package, $value);
    }
    ...
    foreach($packageHashs as $package => $value){
        ...
        $g_res_obj[] = Bd_Webapp_Reader::getData($package, $diffs);
    }
    Bd_Webapp_Reader::setHeader();
    echo json_encode($g_res_obj);
